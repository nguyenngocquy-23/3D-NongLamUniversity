...
//    }
    ...
    public List<HotspotMediaResponse> getMediaByNodeId(int nodeId) {
        String sql = "SELECT h.id, h.nodeId,h.type, h.iconId, h.positionX, h.positionY, h.positionZ, " + "h.pitchX, h.yawY, h.rollZ, h.scale, h.color, h.backgroundColor, h.allowBackgroundColor, h.opacity" + ", m.mediaType, m.mediaUrl, m.caption, m.cornerPointList " + "FROM hotspots AS h JOIN hotspot_medias " + "AS m ON h.id = m.hotspotId WHERE h.nodeId = :nodeId";
    ...
    public List<HotspotNavigationResponse> getNavigationByNodeId(int nodeId) {
        String sql = "SELECT h.id, h.nodeId, h.type, h.iconId, h.positionX, h.positionY, h.positionZ, " + "h.pitchX, h.yawY, h.rollZ, h.scale, h.color, h.backgroundColor, h.allowBackgroundColor, h.opacity" + ", n.targetNodeId " + "FROM hotspots AS h JOIN hotspot_navigations AS n ON h.id = n.hotspotId WHERE h.nodeId = :nodeId";
        return ConnectionPool.getConnection().withHandle(handle -> {
    ...
    public List<HotspotInformationResponse> getInformationByNodeId(int nodeId) {
        String sql = "SELECT h.id, h.nodeId, h.type, h.iconId, h.positionX, h.positionY, h.positionZ, " + "h.pitchX, h.yawY, h.rollZ, h.scale, h.color, h.backgroundColor, h.allowBackgroundColor, h.opacity" + ", i.title, i.content " + "FROM hotspots AS h JOIN hotspot_informations AS i ON h.id = i.hotspotId WHERE h.nodeId = :nodeId";
        return ConnectionPool.getConnection().withHandle(handle -> {
    ...
    public int updateNavHotspots(List<HotspotNavUpdateRequest> navHotspots, int nodeId) {
        String sqlUpdateHotspot = "UPDATE hotspots SET " + "type = :type, iconId = :iconId, positionX = :posX, positionY = :posY, positionZ = :posZ, " + "pitchX = :pitchX, yawY = :yawY, rollZ = :rollZ, scale = :scale, " + "color = :color, backgroundColor = :backgroundColor, " + "allowBackgroundColor = :allowBackgroundColor, opacity = :opacity " + "WHERE id = :id";
